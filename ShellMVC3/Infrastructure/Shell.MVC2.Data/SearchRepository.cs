...
namespace Shell.MVC2.Data
{
    ...
    //TO DO take the matches are refacotr into search
    public class SearchRepository : MemberRepositoryBase
    {
       ...
       public SearchRepository( IMailRepository _mailrepository, AnewluvContext datingcontext)
            : base(datingcontext)
        {
...
//                   if (viewerprofileid != null) viewerprofile = Api.MemberService.getprofiledatabyprofileid (new ProfileModel { profileid = viewerprofileid.GetValueOrDefault() });
...
//                   profile profile = Api.MemberService.getprofilebyprofileid (new ProfileModel { profileid = modeltomap.id }); //db.profiledatas.Include("profile").Include("SearchSettings").Where(p=>p.ProfileID == ProfileId).FirstOrDefault();
...
//                       model.distancefromme = Api.GeoService.getdistancebetweenmembers(
...
//                   model.lastloggedonstring = Api.MemberService.getlastloggedinstring(model.lastlogindate.GetValueOrDefault().ToString());
//                   model.mycatchyintroline = profile.profiledata.mycatchyintroLine;
//                   model.aboutme = profile.profiledata.aboutme;
//                   model.online = Api.MemberService.getuseronlinestatus(new ProfileModel { profileid = profile.id });
...
//                   if (allphotos == true)
//                   {
//                       model.profilephotos.ProfilePhotosApproved = Api.PhotoService.getpagedphotomodelbyprofileidandstatus(profile.id, photoapprovalstatusEnum.Approved, photoformatEnum.Thumbnail, page, ps);   //membereditRepository.GetPhotoViewModel(Approved, NotApproved, Private, MyPhotos);
//                   }// approvedphotos = Api.PhotoService.
//                   else
//                   {
//                       model.profilephotos.SingleProfilePhoto = Api.PhotoService.getgalleryphotomodelbyprofileid(profile.id, photoformatEnum.Thumbnail);
//                   }
//                   return model;
//               }
//               return null;
...
//               new ErroLogging(logapplicationEnum.MemberMapperService).WriteSingleEntry(logseverityEnum.CriticalError,globals.getenviroment, ex, viewerprofileid, null);
...
//            {
//                //profiledata viewerprofile = new profiledata();
//               // if (viewerprofileid != null) viewerprofile = Api.MemberService.getprofiledata(viewerprofileid.GetValueOrDefault());
...
//                // DatingService db = new DatingService();
//                //  MembersRepository membersrepo=  new MembersRepository();
//                profilemetadata metadata = Api.MemberService.getprofilebyprofileid(new ProfileModel { profileid = profileId }).profilemetadata;
...
//                    //  CriteriaModel.BasicSearchSettings. = Api.GeoService.getcountrynamebycountryid((byte)metadata.profile.profiledata.countryid);  //TO DO allow a range of countries to be selected i.e multi select
...
//            {
//                //instantiate logger here so it does not break anything else.
//                new ErroLogging(logapplicationEnum.MemberMapperService).WriteSingleEntry(logseverityEnum.CriticalError,globals.getenviroment, ex, profileId, null);
//                //logger.WriteSingleEntry(logseverityEnum.CriticalError,globals.getenviroment, ex, profileid, null);
//                //log error mesasge
//                //handle logging here
//                var message = ex.Message;
...
//            {
//                //instantiate logger here so it does not break anything else.
//                new ErroLogging(logapplicationEnum.MemberMapperService).WriteSingleEntry(logseverityEnum.CriticalError,globals.getenviroment, ex,profilemodel.profileid , null);
//                //logger.WriteSingleEntry(logseverityEnum.CriticalError,globals.getenviroment, ex, profileid, null);
//                //log error mesasge
//                //handle logging here
//                var message = ex.Message;
...
//                {                     
//                    model.myquicksearch.myselectedcountryname =  Api.GeoService.getcountryidbycountryname(profilemodel.Country) == 0 ? "United States" : profilemodel.Country; //use same country for now
//                }
//                else
//                {
...
//            {
//                //instantiate logger here so it does not break anything else.
//                new ErroLogging(logapplicationEnum.MemberMapperService).WriteSingleEntry(logseverityEnum.CriticalError,globals.getenviroment, ex, membersmodel.profile_id , null);
//                //logger.WriteSingleEntry(logseverityEnum.CriticalError,globals.getenviroment, ex, profileid, null);
//                //log error mesasge
//                //handle logging here
//                var message = ex.Message;
...
//        {
//            return CachingFactory.MembersViewModelHelper.updatememberdata(model);
//        }
//        public MembersViewModel updatememberdatabyprofileid(int profileid)
//        {
//            return CachingFactory.MembersViewModelHelper.updatememberprofiledatabyprofile(profileid);
//        }
//        public bool updateguestdata(MembersViewModel model)
//        {
//            return CachingFactory.MembersViewModelHelper.updateguestdata(model);
//        }
...
//        {
//            return CachingFactory.MembersViewModelHelper.getguestdata(sessionid);
//        }
//        public MembersViewModel getmemberdata(int profileid)
//        {
//           return CachingFactory.MembersViewModelHelper.getmemberdata(profileid  );
//        }
...
//            //var myProfile = Api.MemberService.GetprofiledataByProfileID(ProfileID).profile;
//            // var perfectmatchsearchsettings = Api.MemberService.GetPerFectMatchSearchSettingsByProfileID(ProfileID);
//            // model.Profile = myProfile;
//            //Profile data will be on the include
//            profile = Api.MemberService.getprofilebyprofileid(new ProfileModel { profileid = Convert.ToInt16(ProfileID) });
...
//                model.mycountryname = "United States";// Api.GeoService.getcountrynamebycountryid(profile.profiledata.countryid);
//#else
//            model.mycountryname = Api.GeoService.getcountrynamebycountryid(profile.profiledata.countryid.GetValueOrDefault());
//#endif
...
//                // model = Api.MemberService.getdefaultquicksearchsettingsmembers(model);
...
//                   // model.profiledata = Api.MemberService.getprofiledata(profile.id);
...
//            {
//                //instantiate logger here so it does not break anything else.
//                new ErroLogging(logapplicationEnum.MemberMapperService).WriteSingleEntry(logseverityEnum.CriticalError,globals.getenviroment, ex, null, null);
//                //logger.WriteSingleEntry(logseverityEnum.CriticalError,globals.getenviroment, ex, profileid, null);
//                //log error mesasge
//                //handle logging here
//                var message = ex.Message;
...
//                    // city = db.fnTruncateString(x.city, 11),
//                    lastloggedonstring = Api.MemberService.getlastloggedinstring(x.lastlogindate.GetValueOrDefault()),
//                    lastlogindate = x.lastlogindate,
//                    distancefromme = Api.GeoService.getdistancebetweenmembers((double)x.latitude, (double)x.longitude, myLattitude.Value, myLongitude.Value, "Miles"),
//                    galleryphoto = Api.PhotoService.getgalleryphotomodelbyprofileid(x.id, photoformatEnum.Thumbnail),
//                    lookingforagefrom = x.lookingforagefrom,
//                    lookingForageto = x.lookingForageto,
//                    online = Api.MemberService.getuseronlinestatus(new ProfileModel { profileid = x.id })
...
//                profile profile = new profile();
//                profile = Api.MemberService.getprofilebyprofileid(new ProfileModel { profileid = Convert.ToInt16(profilemodel.profileid) });
//                MembersViewModel model = mapmember((int)profilemodel.profileid);
